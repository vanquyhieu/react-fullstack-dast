  ...
  (set) => ({
    ...
    login: async (email: string, password: string) => {
      try {
        ...
        const response = await axiosClient.post(config.urlAPI+'/v1/auth/loginAdmin', { email, password });
        ...
        if (response && response.status === 200) {
          ...
          const {data} = await axiosClient.get(config.urlAPI+'/v1/auth/getProfileAdmin');
