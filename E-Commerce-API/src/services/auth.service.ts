...
const logout = async(payload: { token: string,
  ...
  const refreshToken = async (customer: {_id: string, email: string})=>{
...
const refreshTokenAdmin = async (employee: {_id: string, email: string})=>{
...
const getProfileAdmin = async (id: string) => {
  // SELECT * FROM employees WHERE id = id
  console.log(id);
